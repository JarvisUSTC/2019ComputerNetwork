...
int main()
{
    ...
    while(1)
    {
        int result = rdt_send(fp,clnt_sock);
        if(result == FULL)
        {
            cout<<"FULL"<<endl;
            rdt_rcv(clnt_sock);
        ...
        {
            if(rdt_rcv(clnt_sock) == nextseqnum)
            {
                cout<<"Finish"<<endl;
                break;
            }
...
int rdt_send(FILE* &fp,int clnt_sock)
{
...
uint8_t rdt_rcv(int clnt_sock)
{
    ...
    if((nCount = read(clnt_sock, buffer, sizeof(package)))>0)
    {
        ...
        return recvPackage->header.btcp_seq;
    }
}
